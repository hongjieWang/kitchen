package com.kitchen.god.sdk.log4j.config;

import com.kitchen.god.sdk.log4j.Level;
import com.kitchen.god.sdk.log4j.helpers.LogLog;
import com.kitchen.god.sdk.log4j.helpers.OptionConverter;
...
public class PropertySetter {
    ...
    public void setProperties(Properties properties, String prefix) {
        ...
        for (Enumeration<?> e = properties.propertyNames(); e.hasMoreElements(); ) {
            ...
            if (key.startsWith(prefix)) {
                if (key.indexOf('.', len + 1) > 0) continue;
                String value = OptionConverter.findAndSubst(key, properties);
                key = key.substring(len);
                if (("layout".equals(key) || "errorhandler".equals(key)) && this.obj instanceof com.kitchen.god.sdk.log4j.Appender)
                    continue;
    ...
    public void setProperty(PropertyDescriptor prop, String name, String value) throws PropertySetterException {
        ...
        if (setter == null) throw new PropertySetterException("No setter for property [" + name + "].");
        Class[] paramTypes = setter.getParameterTypes();
        if (paramTypes.length != 1) throw new PropertySetterException("#params for setter != 1");
        try {
            arg = convertArg(value, paramTypes[0]);
        } catch (Throwable t) {
            throw new PropertySetterException("Conversion to type [" + paramTypes[0] + "] failed. Reason: " + t);
        }
        if (arg == null) throw new PropertySetterException("Conversion to type [" + paramTypes[0] + "] failed.");
    ...
    protected Object convertArg(String val, Class<?> type) {
        ...
        String v = val.trim();
        if (String.class.isAssignableFrom(type)) return val;
        if (int.class.isAssignableFrom(type)) return new Integer(v);
        if (long.class.isAssignableFrom(type)) return new Long(v);
    ...
    protected PropertyDescriptor getPropertyDescriptor(String name) {
        ...
        for (int i = 0; i < this.props.length; i++) {
            if (name.equals(this.props[i].getName())) return this.props[i];
        }
        return null;
    }

    public void activate() {
        if (this.obj instanceof OptionHandler) ((OptionHandler) this.obj).activateOptions();
    }
}
